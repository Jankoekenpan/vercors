 ...
 *
 * Adapted for VerCors, from: https://github.com/KeYProject/verifythis-ltc-2020/blob/master/simplified/Keyserver.java
 */
final public class Keyserver {
    ...
    private int[] keys;
    private int[] codes;
    private int[] unconfirmedKeys;
    private int[] requestType;
    private int count;

    private int REQUESTTYPE_ADD;
    private int REQUESTTYPE_REMOVE;

    //@ ensures inv() ** Perm(REQUESTTYPE_ADD, read) ** Perm(REQUESTTYPE_REMOVE, read);
    public Keyserver() {
        MAXUSERS = 1024;
        emails = new int[MAXUSERS];
        keys = new int[MAXUSERS];
        codes = new int[MAXUSERS];
        unconfirmedKeys = new int[MAXUSERS];
        requestType = new int[MAXUSERS];
        count = 0;
    ...
    /*@
    inline resource inv() =
        // Positive number of max users
        Perm(MAXUSERS, read) ** 0 < MAXUSERS
        // All arrays are non-null and have the same length (the # users)
        ** Perm(emails, read) ** emails != null ** emails.length == MAXUSERS ** Perm(emails[*], write)
        ** Perm(keys, read) ** keys != null ** keys.length == MAXUSERS ** Perm(keys[*], write)
        ** Perm(codes, read) ** codes != null ** codes.length == MAXUSERS ** Perm(codes[*], write)
        ** Perm(unconfirmedKeys, read) ** unconfirmedKeys != null ** unconfirmedKeys.length == MAXUSERS ** Perm(unconfirmedKeys[*], write)
        ** Perm(requestType, read) ** requestType != null ** requestType.length == MAXUSERS ** Perm(requestType[*], write)
        // Number of users is bounded
        ** Perm(count, write) ** 0 <= count ** count <= MAXUSERS
        // Emails are unique
        ** (\forall int i = 0 .. count;
            (\forall int j = 0 .. count; i != j ==> emails[i] != emails[j]))
        ;
     */
    ...
    private int posOfId(int id) {
        ...
        //@ loop_invariant (\forall int k = 0 .. i; emails[k] != id);
    ...
    /*@ ensures (\exists int i; 0 <= i && i < count; emails[i] == id)
            ==> (\exists int i; 0 <= i && i < count; \result == keys[i] && emails[i] == id); */
    //@ signals (Exception e) (\forall int i; 0 <= i && i < count; emails[i] != id);
